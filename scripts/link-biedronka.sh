#!/usr/bin/env bash
# Copy the sibling moja-biedronka integration into Supervisor's HA config.
#
# Must copy (not symlink): Core runs in nested Docker and only sees
# /mnt/supervisor/homeassistant as /config. A link to /mnt/moja-biedronka
# would be broken inside that container. Config dirs are root-owned.
set -euo pipefail

SRC="${BIEDRONKA_SRC:-/mnt/moja-biedronka/custom_components/biedronka}"
if [[ ! -d "$SRC" ]]; then
  echo "Biedronka not found at $SRC"
  echo "Keep the repos as siblings: GIT/moja-biedronka and GIT/homeassistant-browser-companion"
  exit 1
fi
...
DEST="$DEST_PARENT/custom_components/biedronka"
as_root mkdir -p "$DEST_PARENT/custom_components"
as_root rm -rf "$DEST"
as_root cp -a "$SRC" "$DEST"
as_root chmod -R a+rX "$DEST"
echo "Copied $SRC -> $DEST"
echo "Re-run this task after changing Biedronka code."
